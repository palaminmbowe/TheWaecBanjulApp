...
Public Class frmMarks
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            lblTadExaminerNo.Text = TadExaminerNo
    ...
    Sub getname()
        ...
        Try
            ...
            cmd1.CommandText = "select * from details where CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and IndexNo like ('" & cmbIndex.Text & "%')"
    ...
    Sub getthemark()
        ...
        Try
            ...
            cmd.CommandText = "select * from TassMarks where ExamYear Like ('" & ExamYear & "%') and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and IndexNo like ('" & cmbIndex.Text & "%')and SubjCode like ('" & subj & "%')"
    ...
    Sub AddRecord()
        Try
            ...
            db.cmd.CommandText = "Insert Into TassMarks (ExamYear, CentNo, IndexNo,SubjCode, Mark,ExaminerNo,ModiDate) Values  (" & _
                "'" & ExamYear & "', '" & Strings.Left(cmbCenter.Text, 5) & "', '" & cmbIndex.Text & _
    ...
    Sub UpdateRecord()
        Try
            ...
            db.cmd1.CommandText = "UPDATE ExpandedEntry SET Flag='" + txtMark.Text + "'" _
                  & " WHERE ((ExamYear Like ('" & ExamYear & "%'))and (CentNo='" + Strings.Left(cmbCenter.Text, 5) + "') and (IndexNo='" + cmbIndex.Text + "') and (SubjCode='" + lblPaperName.Text + "'))"
    ...
    Sub AuditRecord()
        ...
        Try
            ...
            Using cmd As New OleDb.OleDbCommand("Insert Into AuditTrail (ExamYear, CentNo, IndexNo,SubjCode, Mark,ExaminerNo,CreateDate) Values  (@ExamYear, @centno, @IndexNo,@SubjCode, @Mark,@ExaminerNo,@CreateDate)", db.con)
                ...
                cmd.Parameters.Add(New OleDb.OleDbParameter("@centno", OleDbType.VarChar)).Value = Strings.Left(cmbCenter.Text, 5)
    ...
    Sub getMark()
        ...
        Try
            ...
            center = cmbCenter.SelectedText
            index = cmbIndex.Text
            cmd.CommandText = "select * from TassMarks where ExamYear Like ('" & ExamYear & "%') and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and IndexNo like ('" & cmbIndex.Text & "%') and SubjCode like ('" & lblPaperName.Text & "%')"
            cmd.Connection = gcon
            dr = cmd.ExecuteReader
            If dr.Read() Then
                txtMark.Text = dr("Mark")
    ...
    Function getMaxMark() As Boolean
        ...
        Try
            ...
            Dim index As String
            gcon.Open()
            center = Strings.Left(cmbCenter.SelectedItem.ToString(), 5)
            index = cmbIndex.Text
    ...
    Sub CheckCompletemarks()
        Dim cmd As New OleDbCommand
        con.Open()
        cmd.CommandText = "select Count(*) from CassMarks where ExamYear Like ('" & ExamYear & "%')and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%')"
        cmd.Connection = con
    ...
    Sub CheckExpectedSubjects()
        Try
            ...
            center = cmbCenter.SelectedText
            index = cmbIndex.Text
            cmd.CommandText = "select * from Cand_subjects where ExamYear Like ('" & ExamYear & "%')and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and IndexNo LIKE ('" & cmbIndex.Text & "%')"
            cmd.Connection = gcon
    ...
    Sub CheckCompletedSubjects()
        Dim cmd As New OleDbCommand
        con.Open()
        cmd.CommandText = "select Count(*) from CassMarks where ExamYear Like ('" & ExamYear & "%')and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and IndexNo LIKE ('" & cmbIndex.Text & "%')"
        cmd.Connection = con
        num = cmd.ExecuteScalar
        con.Close()
    End Sub
    ...
    Sub nextindex()
        ...
        cmd.CommandText = "select * from Transact where ExamYear Like ('" & ExamYear & "%') and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and IndexNo like ('" & cmbIndex.Text & "%')and Subject like ('" & subj & "%')"
    ...
    Sub getcenters()
        ...
        cmd.CommandText = "select CentNo from wacenters where ExamYear Like '" & ExamYear & "%' CentNo Like '" & Strings.Left(cmbCenter.Text, 5) & "%'"
        cmd.Connection = con
    ...
    Sub popsubject()
        ...
        Try
            ...
            db.cmd.CommandText = "SELECT DISTINCT ExpandedEntry.SubjCode, Wapapers.PaperName, Wapapers.PaperType " & _
                "FROM ExpandedEntry INNER JOIN Wapapers ON left(ExpandedEntry.SubjCode, 6) = left(Wapapers.PaperCode, 6) " & _
    ...
    Sub getindexes()
        ...
        Else
            ...
            Try
                ...
                db.cmd.CommandText = "select distinct(IndexNo) from ExpandedEntry where  CentNo like('" & Strings.Left(cmbCenter.Text, 5) & "%') and SubjCode like('" & subja & "%')and Flag Like ('" & flag & "%')"
        ...
        If i <> 1 Then
            MessageBox.Show("You have finished your allocation for " + CmbSubject.Text & vbNewLine & vbNewLine & _
                   "Please select a different centre if you have more other centres to key their marks.", _
    ...
    Sub getpapers()
        ...
        Else
            ...
            cmd.CommandText = "select * from wsubjndx where ExamYear Like ('" & ExamYear & "%')and CentNo Like ('" & Strings.Left(cmbCenter.Text, 5) & "%') and Subject like ('" & subj & "%')and flag like ('" & flag & "%')"
            cmd.Connection = gcon
    ...
    Sub getcent()
        Try
            ...
            db.cmd.CommandText = "select Distinct ExpandedEntry.CentNo, Wacenters.Description from ExpandedEntry " & _
                "Inner Join Wacenters on (ExpandedEntry.CentNo = Wacenters.CentNo) " & _
    ...
    Sub Countmarksoustanding()
        Try
            ...
            db.cmd.CommandText = "select count (*) from ExpandedEntry where ExaminerNo=('" & curuser & "') and CentNo='" & Strings.Left(cmbCenter.Text, 5) & "'and SubjCode='" & Strings.Left(CmbSubject.Text, 6) & "' and Flag='" & Flag & "' "
            TextBox2.Text = db.cmd.ExecuteScalar().ToString()
    ...
    Sub UpdateExpandedRecord()
        Try
            ...
            db.cmd.CommandText = "UPDATE ExpandedEntry SET Flag='" + txtMark.Text + "'" _
                  & " WHERE ((ExamYear Like ('" & ExamYear & "%'))and (CentNo='" + Strings.Left(cmbCenter.Text, 5) + "') and (IndexNo='" + cmbIndex.Text + "') and (SubjCode='" + Strings.Left(CmbSubject.Text, 6) + "') and ExaminerNo=('" & curuser & "'))"
    ...
    '        '--------------------------------------
    '        db.cmd4.CommandText = "UPDATE TassMarks INNER JOIN TassMarksServer ON (TassMarks.SubjCode = TassMarksServer.SubjCode) AND (TassMarks.IndexNo = TassMarksServer.IndexNo) AND (TassMarks.CentNo = TassMarksServer.CentNo) AND (TassMarks.ExamYear = TassMarksServer.ExamYear) SET TassMarks.updated = False " & _
    '                "WHERE ((val(TassMarks.Mark)<>val([TassMarksServer].[Mark])));"
    '        If (db.cmd4.ExecuteNonQuery()) Then
    '            'success with local
    '            Console.WriteLine("Updated updates")
    '        End If
