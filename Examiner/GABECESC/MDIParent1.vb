...
Public Class MDIParent1
    ...
    ''"\\Server1\WAEC Applications\GABECE EMS" & "\emsgabece.mdb;Jet OLEDB:Database Password=Sawat1me;"


    Public connectionString As String = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & _
            "C:\EMS\GABECE\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb;Jet OLEDB:Database Password=Sawat1me;"

    'connection for Private WASSCE EMS
    '      "\\server1\Shares\WAEC Applications\PWASSCE EMS" & "\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb;Jet OLEDB:Database Password=Sawat1me;"

    'Public connectionStringServer As String = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & _
    '"\\Server1\WAEC Applications\GABECE EMS" & "\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb;Jet OLEDB:Database Password=Sawat1me;"
    Public connectionStringServer As String = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" &
    "\\svictd\WAEC Applications\GABECE EMS" & "\emsgabece.mdb;Jet OLEDB:Database Password=Sawat1me;"

    Public connectionStringLocal As String = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & _
            System.Environment.CurrentDirectory.ToString() & "\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb;Jet OLEDB:Database Password=Sawat1me;"

    Public connectionStringLocalLocal As String = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & _
            "C:\EMS\GABECE\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb;Jet OLEDB:Database Password=Sawat1me;"

    'Public serverFile As New FileInfo("\\Server1\WAEC Applications\GABECE EMS\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb")
    Public serverFile As New FileInfo("\\svictd\WAEC Applications\GABECE EMS\emsgabece.mdb")
    Public localFile As New FileInfo("C:\EMS\GABECE\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb")
    ...
    Private Sub MDIParent1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Catch ex As Exception
            ...
            If (MessageBox.Show("Error connecting to local db. Please check whether file exist. Do you want to connect to server?",
                                "Error", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            End If
    ...
    Private Function SetupFileInfos() As Boolean
        Try
            'serverFile = New FileInfo("\\Server1\WAEC Applications\GABECE EMS\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb")
            'localFile = New FileInfo("C:\EMS\GABECE\emsgabece" & DateAndTime.Year(Now()).ToString() & ".mdb")
            ...
            If (serverFile.Exists) Then
                ...
                End If
            Else
                MessageBox.Show("Cannot connect to server. Please check connection and try again!", "Cannot Connect", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        Catch ex As Exception
    ...
    '        '--------------------------------------
    '        db.cmd4.CommandText = "UPDATE TassMarks SET TassMarks.updated = False;"
    '        If (db.cmd4.ExecuteNonQuery()) Then
    '            'success with local
    '            Console.WriteLine("Updated updates")
    '        End If
