		...
		private Boolean generateConnectionString()
		{
			...
			try
			{
					...
					MessageBoxDefaultButton.Button1) == DialogResult.Yes))
				{
					...
					try
					{
						dbFile = new System.IO.FileInfo(String.Concat(Properties.Resources.dbServerPathLocal, "\\", Properties.Resources.dbName));
						if (dbFile.Exists)
						{
				...
				dbFile = new System.IO.FileInfo(String.Concat(Properties.Resources.dbServerPath, "\\", Properties.Resources.dbName));
				if (dbFile.Exists)
				{
				...
				else
				{
					...
					try
					{
						...
						if (dbFile.Exists)
						{
							//lblHeader.Text += " LOCAL";
							lblHeader.ForeColor = Color.Red;
							db = new dbConnection3(Properties.Resources.dbName, Properties.Resources.dbServerPathLocal, getPassword());
							return db.isConnected();
						}
						else
						{
					...
					switch (exam)
					{
						...
						case Exams.WASSCE:
                            if(chkbGroupBySubjects.Checked == true)
                            {
								...
								//crGpw3Sub.Refresh();
								////crystalReportViewer1 = new CrystalReportViewer();
								//crystalReportViewer1.ReportSource = crGpw3Sub;

								crGpw4 = new crpPrePrintObjsGPW4();
								crGpw4.Database.Tables["qryPrePrintObj"].SetDataSource((DataTable)ds.Tables["qryPrePrintObj"]);
								crGpw4.VerifyDatabase();
								crGpw4.Refresh();
								//crystalReportViewer1 = new CrystalReportViewer();
								crystalReportViewer1.ReportSource = crGpw4;
							}
